...
const MACRO_DISPLAY = ['SPY', 'RSP', 'IWM', 'DX-Y.NYB', '^VIX', 'BTC-USD', 'GC=F', 'CL=F'];
const SENTIMENT_TICKERS = ['SPHB', 'ARKK', 'IBIT', 'MEME', 'GLD', 'TLT', 'XLU', 'XLP', 'HYG', 'IEF', '^VIX3M',
  'HG=F', '^TNX', '^GSPC'  /* Doctor Copper, 10Y Yield, S&P 500 index for SMA */
];
const ALL_EXTRA_TICKERS = [...MACRO_DISPLAY, ...SENTIMENT_TICKERS];

const MACRO_INFO = {
  'SPY': { label: 'S&P 500 (SPY)', icon: '📈' },
  'RSP': { label: 'S&P 500 Eq Wgt (RSP)', icon: '⚖️' },
  'IWM': { label: 'Small Caps (IWM)', icon: '🏭' },
  'DX-Y.NYB': { label: 'US Dollar Index (DXY)', icon: '💵' },
  '^VIX': { label: 'Volatility (VIX)', icon: '📉', invert: true },
  'BTC-USD': { label: 'Bitcoin (BTCUSD)', icon: '₿' },
  'GC=F': { label: 'Gold (GC=F)', icon: '🪙' },
  'CL=F': { label: 'Crude Oil (CL=F)', icon: '🛢️' },
};
...
export default function App() {
  ...
  const loadData = async (force = false) => {
    try {
      setError(null);
      const data = await fetchAllQuotes(CATEGORIES, ALL_EXTRA_TICKERS, timeframe, force);
      setQuoteMap(data);
    ...
    <div className="app-layout">
      ...
      <main className="main-content">
        ...
        <div className="content-wrapper">
          ...
          {!loading && quoteMap.size > 0 && activeTool !== 'Sentiment' && (
            <div className="summary-bar">
              {MACRO_DISPLAY.map((sym) => {
          ...
          {loading && quoteMap.size === 0 ? (
            ...
            <>
              ...
              {activeTool === 'Sentiment' && (
                <SentimentDashboard quoteMap={quoteMap} />
              )}
            </>
          )}
        </div>
